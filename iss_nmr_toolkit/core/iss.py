...
def run_inference(policy: Any, observations: Sequence[Observation], batch_size: int, desc: str) -> list[dict[str, Any]]:
    ...
    def stack_obs(key):
        data = np.stack(observations[key].to_numpy())
        data = np.expand_dims(data, axis=1)
        # permute dim 0 dim 1
        return data

    input_obs: Dict[str, Dict[str, torch.Tensor]] = {}

    for key in observations:
        split_keys = key.split('.')
        if split_keys[0] in ['state', 'video']:
            if split_keys[0] not in input_obs:
                input_obs[split_keys[0]] = {}
            input_obs[split_keys[0]][split_keys[1]] = stack_obs(key=key)
        elif split_keys[0] == 'language':
            input_obs['language'] = {}
            input_obs['language']['annotation.human.action.task_description'] = [[observations[key][0]]] * len(observations['state.x'])

    predicted_action, info = policy.get_action(input_obs)

    keys = ["x", "y", "z", "roll", "pitch", "yaw", "gripper"]
    action = torch.cat([torch.tensor(predicted_action[k]) for k in keys], dim=-1)
    actions = list(action.unbind(dim=0))

    for action in actions:
        results.append({'actions': action})
...
def compute_iss(
    ...
    *,
    ...
    total_steps = len(observations['video.image'])
